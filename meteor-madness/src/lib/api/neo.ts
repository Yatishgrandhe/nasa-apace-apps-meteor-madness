...
function generateFallbackCometData(): CometObject[] {
  ...
  for (let i = 0; i < 10; i++) {
    ...
    fallbackComets.push({
      ...
      approach_date: new Date(Date.now() + Math.random() * 730 * 24 * 60 * 60 * 1000).toISOString().split('T')[0],
      is_hazardous: Math.random() < 0.15,
      magnitude: Math.random() * 12 + 12,
      orbit_class: ['Apollo', 'Aten', 'Amor', 'Atira', 'Long Period', 'Short Period', 'Halley-type', 'Jupiter Family'][Math.floor(Math.random() * 8)],
      last_observed: new Date(Date.now() - Math.random() * 365 * 24 * 60 * 60 * 1000).toISOString().split('T')[0]
    })
  }
